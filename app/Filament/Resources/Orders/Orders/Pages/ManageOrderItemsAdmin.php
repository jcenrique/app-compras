...
class ManageOrderItemsAdmin extends ManageRelatedRecords
{
    ...
    public function getTitle(): string
    {
        ...
        return __('common.order_resource_label').': '.self::getRecord()->client->name.' -> '.
             Carbon::createFromDate(self::getRecord()->order_date)->format('d/M/Y');
    ...
    public function getTabs(): array
    {
        ...
        return [
            'pending' => Tab::make()->label(__('common.order_statuses.pending'))->icon('fas-hourglass')
                ->modifyQueryUsing(fn ($query) => $query->where('is_basket', false))
            ...
            'in_basket' => Tab::make()->label(__('common.is_basket'))->icon('fas-shopping-basket')
                ->modifyQueryUsing(fn ($query) => $query->where('is_basket', true))
                        ...
                        if ($state) {
                            ...
                            if ($allItemsInBasket === $record->order->items()->count()) {
                                ...
                                Notification::make()
                                    ...
                                    ->persistent()
                                    ->send();
                                redirect(ManageOrderItemsAdmin::getUrl(['record' => $record->order->id]));
                            }
                            $this->getTabs();
